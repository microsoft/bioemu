...
class TestCaCaDistance:
    ...
    def test_known_distances(self):
        ...
        for i in range(4):
            ca_pos[0, i, 0] = i * 3.8
        result = cv.compute_batch(ca_pos)
        torch.testing.assert_close(result, torch.full((1, 3), 3.8), atol=1e-5, rtol=1e-5)
...
class TestPairwiseClash:
    ...
    def test_no_clash(self):
        ...
        cv = PairwiseClash(min_dist=4.0, offset=3)
        ca_pos = torch.zeros(1, 10, 3)
        for i in range(10):
            ca_pos[0, i, 0] = i * 10.0  # well separated
        result = cv.compute_batch(ca_pos)
    ...
    def test_monotonic_with_distance(self):
        ...
        cv = PairwiseClash(min_dist=4.0, offset=1)
        energies = []
        for spacing in [0.5, 1.0, 2.0, 5.0, 10.0]:
