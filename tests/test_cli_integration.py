...
def test_basic_readme_command():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        cmd = [
            ...
            "--sequence", "GYDPETGTWG",
            "--num_samples", "5",  # Small number for fast testing
...
def test_steering_cli_integration():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        steering_config_path = Path(__file__).parent.parent / "src" / "bioemu" / "config" / "steering" / "physical_potentials.yaml"
        ...
        cmd = [
            ...
            "--num_samples", "5",  # Small number for fast testing
            "--output_dir", output_dir,
            "--steering_potentials_config", str(steering_config_path),
            "--num_steering_particles", "2",
...
def test_steering_parameter_verification():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        cmd = [
            sys.executable, "-m", "bioemu.sample",
            "--sequence", "GYDPETGTWG",
            "--num_samples", "3",  # Small number for fast testing
            "--output_dir", output_dir,
            "--num_steering_particles", "4",  # Use 4 particles to make batch size change obvious
            "--steering_start_time", "0.7",
            "--steering_end_time", "0.95",
            "--resampling_freq", "2",
            "--fast_steering", "False"
        ]
