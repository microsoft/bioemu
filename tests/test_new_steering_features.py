...
def test_physical_steering_flag(chignolin_sequence, base_test_config, physical_steering_config):
    ...
    print(f"\n🧪 Testing physical_steering flag with {chignolin_sequence}")
    ...
    print(f"✅ Physical steering completed: {samples['pos'].shape[0]} samples with auto potentials")
...
def test_fast_steering_performance(chignolin_sequence, base_test_config, physical_steering_config):
    ...
    print(f"\n🧪 Testing fast_steering performance with {chignolin_sequence}")
...
def test_steering_assertion_validation(chignolin_sequence, base_test_config):
    ...
    with pytest.raises(AssertionError, match="Steering was expected but never executed"):
        sample(
            sequence=chignolin_sequence,
            num_samples=5,
            batch_size_100=50,
...
def test_steering_end_time_window(chignolin_sequence, base_test_config):
    ...
    print(f"\n🧪 Testing steering end time window with {chignolin_sequence}")
    ...
    print("✅ Steering end time window test passed - steering executed within specified window")
